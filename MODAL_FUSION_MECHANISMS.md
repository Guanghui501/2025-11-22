# 多模态特征融合机制详解

本文档详细说明在不同模态搭配作用下，图特征和文本特征的变化过程。

---

## 📊 整体架构流程

```
文本输入 → MatSciBERT编码 → CLS投影 → 跨模态交互 → 融合预测
   ↓                                        ↑
图结构 → 原子嵌入 → ALIGNN层 → GCN层 → 图投影
```

---

## 1️⃣ 基础特征提取阶段（无交互）

### 图特征流程
**位置**: `alignn.py:883-903`

```python
# 1. 原子特征嵌入 [total_atoms, 92] → [total_atoms, 256]
x = self.atom_embedding(x)

# 2. ALIGNN层更新（4层）：节点-边-角度特征联合更新
for alignn_layer in self.alignn_layers:
    x, y, z = alignn_layer(g, lg, x, y, z)

# 3. GCN层更新（4层）：节点-边特征更新
for gcn_layer in self.gcn_layers:
    x, y = gcn_layer(g, x, y)

# 4. 图级表示 [total_atoms, 256] → [batch, 256] → [batch, 64]
graph_emb = self.readout(g, x)  # 平均池化
h = self.graph_projection(graph_emb)  # 投影到64维
```

**特征维度变化**:
- 输入: `[total_atoms, 92]` (原子特征)
- 嵌入后: `[total_atoms, 256]`
- 池化后: `[batch_size, 256]`
- **最终图特征**: `[batch_size, 64]` ✅

### 文本特征流程
**位置**: `alignn.py:866-880`

```python
# 1. 文本标准化
norm_sents = [normalize(s) for s in text]

# 2. MatSciBERT编码 [batch, seq_len] → [batch, seq_len, 768]
encodings = tokenizer(norm_sents, return_tensors='pt', padding=True)
last_hidden_state = text_model(**encodings)[0]

# 3. 提取CLS token + 投影 [batch, 768] → [batch, 64]
cls_emb = last_hidden_state[:, 0, :]
text_emb = self.text_projection(cls_emb)
```

**特征维度变化**:
- BERT输出: `[batch_size, seq_len, 768]`
- CLS token: `[batch_size, 768]`
- **最终文本特征**: `[batch_size, 64]` ✅

**此阶段特点**:
- ✅ 图和文本**完全独立**编码，无交互
- ✅ 两者都被投影到相同维度空间（64维）
- ✅ 这是 `visualize_latent_space.py` 中 `'graph'` 和 `'text'` 特征的来源

---

## 2️⃣ 中间融合机制 (Middle Fusion)

**启用条件**: `config.use_middle_fusion = True`
**位置**: `alignn.py:121-218`, `alignn.py:896-899`

### 作用时机

在 **ALIGNN 层内部**进行融合（图编码过程中）：

```python
# ALIGNN层循环
for idx, alignn_layer in enumerate(self.alignn_layers):
    x, y, z = alignn_layer(g, lg, x, y, z)  # 图更新

    # 如果当前层配置了中间融合（如第2层）
    if idx in self.middle_fusion_layer_indices:  # 例如 idx=2
        x = self.middle_fusion_modules[f'layer_{idx}'](x, text_emb, batch_num_nodes)
```

### 融合机制详解

**MiddleFusionModule 内部流程**:

```python
# 输入:
#   node_feat: [total_atoms, 256] - 当前层的节点特征
#   text_feat: [batch, 64] - 文本特征（已投影）

# 1. 文本特征变换 [batch, 64] → [batch, 256]
text_transformed = self.text_transform(text_feat)

# 2. 广播文本特征到每个原子
#    根据每个图的原子数，将文本特征复制给对应的原子
#    [batch, 256] → [total_atoms, 256]
text_broadcasted = 按图重复(text_transformed, batch_num_nodes)

# 3. 门控融合机制
gate_input = concat([node_feat, text_broadcasted], dim=-1)  # [total_atoms, 512]
gate_values = sigmoid(Linear(gate_input))  # [total_atoms, 256]

# 4. 门控更新 + 残差连接
enhanced = node_feat + gate_values * text_broadcasted
enhanced = LayerNorm(Dropout(enhanced))

# 输出: [total_atoms, 256] - 文本增强的节点特征
```

### 特征变化

**对图特征（节点特征）的影响**:
```
第2层ALIGNN输出: [total_atoms, 256]
    ↓
文本信息注入（门控加权）
    ↓
文本增强的节点特征: [total_atoms, 256]
    ↓ 继续后续ALIGNN/GCN层
最终图特征: [batch, 64]
```

**对文本特征的影响**:
- ❌ **不直接改变文本特征**
- ✅ 文本通过门控机制调制图编码过程
- ✅ 文本信息隐式地融入到图的节点表示中

### 门控机制效果

**门控值的含义**:
```python
gate_values = sigmoid(Linear([node_feat, text_feat]))  # 范围 [0, 1]
```

- `gate_values ≈ 1`: 该原子节点**强烈接受**文本信息
- `gate_values ≈ 0`: 该原子节点**忽略**文本信息
- 每个原子根据自身特征和文本内容**自适应决定**接受多少文本影响

**示例**:
```
文本: "This is a high-temperature superconductor with copper-oxygen layers"

原子特征 + 门控值:
Cu原子: gate=0.9 → 强烈接受文本中"copper"的语义信息
O原子:  gate=0.8 → 接受"oxygen"相关信息
Si原子: gate=0.2 → 弱接受（文本未提及）
```

### 核心效果

- 🔄 **单向融合**: 文本 → 图（文本调制图编码）
- ⏰ **早期融合**: 在图编码**中间层**介入，不是最后才融合
- 🎯 **原子级精细控制**: 每个原子独立决定接受多少文本信息
- 📍 **局部性**: 文本信息通过后续图卷积层传播到整个图

---

## 3️⃣ 细粒度跨模态注意力 (Fine-grained Cross-modal Attention)

**启用条件**: `config.use_fine_grained_attention = True`
**位置**: `alignn.py:352-528`, `alignn.py:905-952`

### 作用时机

在 **所有图层之后，池化之前**：

```python
# 完成所有ALIGNN和GCN层
for gcn_layer in self.gcn_layers:
    x, y = gcn_layer(g, x, y)
# 此时 x: [total_atoms, 256]

# 细粒度注意力
enhanced_nodes, enhanced_tokens = self.fine_grained_attention(
    node_features_batched,  # [batch, max_atoms, 256]
    text_tokens,            # [batch, seq_len, 768] (BERT所有token)
    node_mask=node_mask,
    token_mask=attention_mask
)

# 继续池化
graph_emb = self.readout(g, x_enhanced)
```

### 双向注意力机制

#### **原子→文本注意力** (Atom-to-Token)

```python
# 1. 计算查询、键、值
Q_a2t = Linear(node_feat)    # [batch, max_atoms, hidden] - 原子查询
K_a2t = Linear(token_feat)   # [batch, seq_len, hidden]   - 文本键
V_a2t = Linear(token_feat)   # [batch, seq_len, hidden]   - 文本值

# 2. 多头注意力得分 [batch, heads, max_atoms, seq_len]
attn_a2t = softmax((Q_a2t @ K_a2t.T) / sqrt(head_dim))

# 3. 加权聚合文本信息
context_a2t = attn_a2t @ V_a2t  # [batch, max_atoms, hidden]

# 4. 残差连接
enhanced_nodes = LayerNorm(node_feat + context_a2t)
```

**含义**: 每个原子关注文本中的哪些词？

**示例**:
```
材料描述: "Silicon dioxide with high thermal stability"
原子序列: [Si₁, O₁, O₂, Si₂, ...]

注意力权重矩阵 [atoms × tokens]:
          Silicon  dioxide  with  high  thermal  stability
Si₁        0.7     0.2     0.0   0.0    0.1      0.0      ← Si原子关注"Silicon"
O₁         0.1     0.6     0.0   0.0    0.2      0.1      ← O原子关注"dioxide", "thermal"
O₂         0.1     0.6     0.0   0.0    0.2      0.1
Si₂        0.7     0.2     0.0   0.0    0.1      0.0
```

#### **文本→原子注意力** (Token-to-Atom)

```python
# 1. 计算查询、键、值
Q_t2a = Linear(token_feat)   # [batch, seq_len, hidden]   - 文本查询
K_t2a = Linear(node_feat)    # [batch, max_atoms, hidden] - 原子键
V_t2a = Linear(node_feat)    # [batch, max_atoms, hidden] - 原子值

# 2. 多头注意力得分 [batch, heads, seq_len, max_atoms]
attn_t2a = softmax((Q_t2a @ K_t2a.T) / sqrt(head_dim))

# 3. 加权聚合原子信息
context_t2a = attn_t2a @ V_t2a  # [batch, seq_len, hidden]

# 4. 残差连接
enhanced_tokens = LayerNorm(token_feat + context_t2a)
```

**含义**: 每个文本词关注结构中的哪些原子？

**示例**:
```
注意力权重矩阵 [tokens × atoms]:
           Si₁   O₁   O₂   Si₂  ...
Silicon    0.8   0.0  0.0  0.7  ...  ← "Silicon"词关注Si原子
dioxide    0.1   0.4  0.4  0.1  ...  ← "dioxide"词关注O原子
thermal    0.2   0.2  0.2  0.2  ...  ← "thermal"词均匀关注
stability  0.15  0.2  0.2  0.15 ...
```

### 特征变化

**对图特征（原子级）的影响**:
```
原始原子特征: [batch, max_atoms, 256]
    ↓
原子→文本注意力
    ↓
文本增强的原子特征: [batch, max_atoms, 256]
    ↓ 转回DGL格式 + 池化
增强的图级特征: [batch, 64]
```

**对文本特征（token级）的影响**:
```
原始token特征: [batch, seq_len, 768]
    ↓
文本→原子注意力
    ↓
结构增强的token特征: [batch, seq_len, 768]
    ↓ (如果需要)可以重新池化
增强的文本特征
```

### 核心效果

- 🔄 **双向融合**: 图 ↔ 文本（互相增强）
- 🔬 **细粒度**: 原子-词级别的精确对齐
- 🎯 **可解释性**: 可以可视化哪个原子关注哪个词
- ⏰ **晚期融合**: 在图编码完成后进行
- 📊 **特征维度不变**: 原子特征仍为256维，token特征仍为768维

---

## 4️⃣ 全局跨模态注意力 (Cross-modal Attention)

**启用条件**: `config.use_cross_modal_attention = True`
**位置**: `alignn.py:221-349`, `alignn.py:960-971`

### 作用时机

在 **图池化和投影之后**（最终融合阶段）：

```python
# 1. 图特征已经池化并投影
graph_emb = self.readout(g, x)      # [batch, 256]
h = self.graph_projection(graph_emb)  # [batch, 64]

# 2. 文本特征已经投影
text_emb = self.text_projection(cls_emb)  # [batch, 64]

# 3. 全局跨模态注意力
enhanced_graph, enhanced_text = self.cross_modal_attention(h, text_emb)

# 4. 平均融合
h = (enhanced_graph + enhanced_text) / 2  # [batch, 64]
out = FC(h)  # 最终预测
```

### 双向注意力机制

#### **图→文本注意力** (Graph-to-Text)

```python
# 输入都是全局特征（每个样本一个向量）
# graph_feat: [batch, 64]
# text_feat: [batch, 64]

# 1. 添加序列维度
graph_feat_seq = graph_feat.unsqueeze(1)  # [batch, 1, 64]
text_feat_seq = text_feat.unsqueeze(1)    # [batch, 1, 64]

# 2. 计算查询、键、值（多头）
Q_g2t = self.g2t_query(graph_feat_seq)  # [batch, 1, 256]
K_g2t = self.g2t_key(text_feat_seq)     # [batch, 1, 256]
V_g2t = self.g2t_value(text_feat_seq)   # [batch, 1, 256]

# 3. 多头注意力
attn_g2t = softmax((Q_g2t @ K_g2t.T) / sqrt(head_dim))  # [batch, heads, 1, 1]
context_g2t = attn_g2t @ V_g2t  # [batch, 1, 256]
context_g2t = Linear(context_g2t)  # [batch, 64]

# 4. 残差 + 归一化
enhanced_graph = LayerNorm(graph_feat + context_g2t)  # [batch, 64]
```

#### **文本→图注意力** (Text-to-Graph)

```python
# 对称操作
Q_t2g = self.t2g_query(text_feat_seq)   # [batch, 1, 256]
K_t2g = self.t2g_key(graph_feat_seq)    # [batch, 1, 256]
V_t2g = self.t2g_value(graph_feat_seq)  # [batch, 1, 256]

attn_t2g = softmax((Q_t2g @ K_t2g.T) / sqrt(head_dim))
context_t2g = attn_t2g @ V_t2g
context_t2g = Linear(context_t2g)  # [batch, 64]

enhanced_text = LayerNorm(text_feat + context_t2g)  # [batch, 64]
```

### 特征变化

**对图特征的影响**:
```
原始图特征: [batch, 64]
    ↓
图→文本注意力（图查询文本信息）
    ↓
文本增强的图特征: [batch, 64]
    ↓
与增强文本特征平均
    ↓
融合特征: [batch, 64] → 用于最终预测
```

**对文本特征的影响**:
```
原始文本特征: [batch, 64]
    ↓
文本→图注意力（文本查询图信息）
    ↓
图增强的文本特征: [batch, 64]
    ↓
与增强图特征平均
    ↓
融合特征: [batch, 64] → 用于最终预测
```

### 注意力权重含义

由于序列长度为1（全局特征），注意力权重本质上是**标量相似度**:

```python
attn_g2t: [batch, heads, 1, 1]  # 图对文本的关注度（接近1.0）
attn_t2g: [batch, heads, 1, 1]  # 文本对图的关注度（接近1.0）
```

**效果**:
- 不是选择性注意（像细粒度那样），而是**全局信息融合**
- 相当于学习一个自适应的融合权重

### 核心效果

- 🔄 **双向融合**: 图 ↔ 文本（互相增强）
- 🌍 **全局级别**: 整个图和整个文本的交互
- ⏰ **最晚融合**: 在所有编码完成后
- 🎯 **对称性**: 图和文本平等地相互增强
- 📊 **维度保持**: 融合后仍为 `[batch, 64]`

---

## 📈 可视化特征对应关系

在 `visualize_latent_space.py` 中提取的特征：

| 特征类型 | 来源位置 | 包含的交互 | 维度 |
|---------|---------|-----------|-----|
| `'graph'` | `graph_features` | 可能包含中间融合、细粒度注意力的影响 | 64 |
| `'text'` | `text_features` | 可能包含细粒度注意力的影响 | 64 |
| `'fused'` | `concat([graph, text])` | 简单拼接上述两者 | 128 |

**注意**:
- 如果 `use_cross_modal_attention=True`，返回的是 `enhanced_graph` 和 `enhanced_text`
- 如果启用了细粒度注意力，图特征已经融合了文本信息

---

## 🎯 三大机制对比总结

| 机制 | 融合时机 | 融合粒度 | 方向 | 特征维度变化 | 主要作用 |
|-----|---------|---------|-----|-------------|---------|
| **中间融合** | ALIGNN层内 | 原子级 | 单向(文→图) | ❌ 不变 | 文本调制图编码 |
| **细粒度注意力** | 图编码后 | 原子-词级 | 双向 | ❌ 不变 | 精细语义对齐 |
| **全局注意力** | 最终融合 | 全局 | 双向 | ❌ 不变 | 全局信息融合 |

---

## 🔬 特征空间变化示意

### 无交互（基础模型）
```
图空间:     ●  ●  ●  ●  ●
                ↓
文本空间:   ○  ○  ○  ○  ○
           (两个独立空间)
```

### 中间融合后
```
文本→图:    ●' ●' ●' ●' ●'
            ↑文本增强的图特征
文本:       ○  ○  ○  ○  ○
           (文本未变)
```

### 细粒度注意力后
```
互相增强:   ●" ●" ●" ●" ●"
            ↕️  ↕️  ↕️  ↕️
            ○" ○" ○" ○"
```

### 全局注意力后
```
最终融合:   ⊕  ⊕  ⊕  ⊕  ⊕
          (图文深度融合的统一表示)
```

---

## 💡 实际应用建议

### 选择融合策略

1. **需要文本引导图编码**:
   - 启用: `use_middle_fusion=True`
   - 适合: 文本描述对结构理解很重要的任务

2. **需要可解释性**:
   - 启用: `use_fine_grained_attention=True`
   - 适合: 需要知道哪些原子对应哪些词

3. **最大融合效果**:
   - 同时启用所有三个机制
   - 适合: 复杂的多模态推理任务

---

## 📝 代码位置索引

- **中间融合定义**: `alignn.py:121-218`
- **中间融合应用**: `alignn.py:896-899`
- **细粒度注意力定义**: `alignn.py:352-528`
- **细粒度注意力应用**: `alignn.py:905-952`
- **全局注意力定义**: `alignn.py:221-349`
- **全局注意力应用**: `alignn.py:960-971`
- **特征提取**: `visualize_latent_space.py:57-157`

---

**文档生成时间**: 2025-11-22
